...
Renderer::GeometryInfo* Renderer::readFullBinary(char* fileName)
{
	std::ifstream File(fileName,std::ios::binary | std::ios::in | std::ios::ate);
	...
	if(File.good())
	{
		...
		return addGeometry(modifier,NumVerts*(12*sizeof(float)),(ushort*)(modifier + NumVerts*(12*sizeof(float))),NumIndices, GL_TRIANGLES);
	}
	else
	{
	return nullptr;
